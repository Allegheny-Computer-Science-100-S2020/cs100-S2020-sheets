...
``Allegheny-Computer-Science-100-Spring-2020/computer-science-100-spring-2020-lab-2-gkapfham''.
...
should now type \command{cd cs100S2020}. Next, you can type the command
...
\begin{lstlisting}
  git clone git@github.com:Allegheny-Computer-Science-100-S2020/computer-science-100-spring-2020-lab-2-gkapfham.git
\end{lstlisting}
...
they are not otherwise overwritten during the completion of the assignment.
%
If the above \command{docker run} command does not work correctly on the Windows
operating system, then you may need to instead run the following command to work
around limitations in the terminal:
...
% NOTE: Did not format correctly after adding the new Docker command for Windows

% \marginnote{\think{Reflect on challenges}}[.25in]
...
today's date, and three more lines of text, as given in Figure~\ref{fig:output}.
Make sure there are spaces separating words in your output (did you forget to
put a space inside the quotation marks after your last name?). If not, then
repair the program and re-build and re-run it. Once the program runs, please
...
should write technical content that reflects on your experiences. You should
focus on building a better understanding of each command and code segment --- if
there is one that you do not understand, please ask someone! For instance, can
you explain the purpose and behavior of these statements?
...
\begin{itemize}
  ...
  \item \program{yardsToMoon = milesToMoon * yardsPerMile;}
...
\marginnote{\caution{Request feedback}}[.6in] Using a report that the instructor
...
assignment. Finally, please remember to read your GitHub repository's
\program{README.md} file for a description of the four grades that you will
receive for this laboratory assignment.
